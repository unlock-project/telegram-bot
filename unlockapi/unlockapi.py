...
class UnlockAPI:
            ...
            async with self.__session.get(
                ...
                if resp.status == 200:
                    if resp.content_type != 'application/json':
                        raise TypeError(f"Non-json response ({resp.content_type}) in function {function}")
                    return await resp.json()
                else:
                    raise TypeError(f"Response status {resp.status} in function {function}")
        except:
            return None
    ...
    async def registration_choose(self, vote_id: int, user_id: int, option_id: int):
        data = await self._post(APIMethods.REGISTRATION_RESPONSE, schemas.OptionBotSchema(vote_id=vote_id,
                                                                                          user_id=user_id,
                                                                                          option_id=option_id))
